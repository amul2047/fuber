...
<section class="content">
	...
	<div class="box">
		<div class="box-header">
			<h3 class="box-title">Add User Story</h3>
		</div>
		<!-- /.box-header -->
		<div class="box-body">
			<tr>
				...
				<div class="row">
					<div class="col-sm-12">
						<table id="example2"
							...
							<thead>
								<tr role="row">
									...
									<th class="sorting" tabindex="0" aria-controls="example2"
										rowspan="1" colspan="1"
										aria-label="Browser: activate to sort column ascending">User Story</th>
									<th class="sorting" tabindex="0" aria-controls="example2"
										rowspan="1" colspan="1"
							...
							<c:forEach items="${userStoryList}" var="loopStory" varStatus="loopCounter">
								<tr role="row" class="odd" id="row-${loopCounter}">
								</tr>
								<tr role="row" class="odd">
									<td class="sorting_1"><c:out value="${loopCounter.count}" /></td>
									<td><c:out value="${loopStory.name}" /></td>
									
									<td>
										<button class="btn btn-success" onclick="editUserStory(${loopStory.id});">
											<i class="fa fa-pencil"></i> Edit
										</button> 
										<a class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this User Story?');"
											href="/mumscrum/userstory/delete/${loopStory.id}"> <i class="fa fa-times"></i>Delete
										</a>	
									</td>
								</tr>
							</c:forEach>
	...
	<!-- Modal -->
	<div class="modal fade" id="addUserStoryModal" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<c:url var="actionUrl" value="${productBacklogId}/saveUserStory" /><br><br>
			
				  <form action="../${actionUrl}" commandName="userstory" role="form" id="modalForm" method="post">
                  <div class="box-body">
                    <div class="form-group">
                      <label for="exampleInputEmail1">Name</label>
                      <textarea path ="name" name="name" class="form-control" id="name" placeholder="Enter User Story name"> </textarea>
                    </div>
...
<script type="text/javascript">
	...
	{
		$.get("/mumscrum/userstory/get/" + id, function(result) {
			
			$("#id").val(result.userstory.id);
			$("#name").val(result.userstory.name);
			$("#description").val(result.userstory.description);
